...
import starBackground from "./canvas.js";
import {
  addQipao,
  setPrizes,
  showPrizeList,
...
function initAll() {
  ...
  window.AJAX({
    ...
    success(data) {
      ...
      //添加判断，如果行数少于5行，列数少于15行，取消高亮
      if(!(ROW_COUNT<5||COLUMN_COUNT<15)){
...
let deepCopy = (source)=>{
  ...
  const desc = Array.isArray(source)?[]:{};
  for(let key in source){
      const value = source[key];
      desc[key] = typeof value === 'object'? deepCopy(value):value;
  }
...
function lottery() {
  rotateBall().then(() => {
    ...
    if (leftCount === 0) {
      ...
      basicData.leftUsers = deepCopy(basicData.users);
      leftCount = basicData.leftUsers.length;
    }
...
window.onload = function () {
  ...
  let rotated = 0,
    ...
    customSettings=document.querySelector("#customSettings");
  ...
  customSettings.addEventListener('click',function(){
