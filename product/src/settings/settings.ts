import "./settings.css";
//import starBackground from "../lottery/canvas.js";
import 'layui-src'
...
function initAll(){
  let mm=requestAnimationFrame(initAll)
  if(flag_prize){
    generateTable(prizes)
...
function generateTable(prizes=[])
{
  let prizeTable=document.getElementById('prizeListTable');
  let prizeListCountTable=document.getElementById('prizeListCountTable');
  let personTable=document.getElementById('personTable');
    ...
    table.render({
      ...
      ,page: true //开启分页
      ,cols: [[ //表头
        {field: 'type', title: 'ID',sort: true,hide:true}
        ,{field: 'text', title: '奖项等级', width:'20%',fixed: 'left'}
        ,{field: 'title', title: '奖品名称', width:'40%', sort: true}
        ,{field: 'count', title: '数量', width:'10%', sort: true}
        ,{field: 'img', title: '图片地址', width:'30%'} 
  ...
  table.on('tool(demo)', function(obj){
    ...
    table.render({
      ...
      ,cols: [[ //表头
        ...
        ,{field: 'city', title: '花名', width:80} 
        ,{field: 'sign', title: '部门', width: 177}
        ,{field: 'experience', title: '职位', width: 80, sort: true}
      ]]
    });
